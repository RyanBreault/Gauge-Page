function drawGauge(id){
  ...
  chart.setOption({
    ...
    tooltip:{
      trigger:"axis",
      backgroundColor:"rgba(7,17,23,.95)",
      borderColor:"#203949",
      textStyle:{color:"#edf7fb"},
    ...
    dataZoom:[
      {type:"inside"},
      {type:"slider",height:18,bottom:8,borderColor:"#203949",backgroundColor:"rgba(255,255,255,.04)"}
    ],
    xAxis:{
      type:"category",
      data:labels,
      boundaryGap:false,
      axisLabel:{color:"#8da8b7"},
      axisLine:{lineStyle:{color:"#203949"}},
      axisTick:{show:false}
    },
    yAxis:[
      {
    ...
    series:[
      {
        ...
        data:flow.map(p => Math.round(p.value))
      },
      ...(temp.length ? [{
        name:"Water Temp",
        type:"line",
        ...
        yAxisIndex:1,
        lineStyle:{width:2},
        data:labels.map(l => tempMap.get(l) ?? null)
      }] : [])
    ]
  });
}
