...
const allGauges = () => RIVERS.flatMap(r => getRiverGauges(r).map(g => ({...g, river:r.name})));
...
function findGauge(id){
  return allGauges().find(g => g.id === id);
}

function getHeadlineGauge(river){
  const gauges = getRiverGauges(river);
  return gauges.find(g => g.id === river.headline) || gauges[0];
}
...
function renderFavoritesPanel(){
  ...
  const favorites = state.favorites.map(findGauge).filter(Boolean);
  ...
  panel.innerHTML = `
    <div class="favorite-title">
      <h2>Favorite Gauges</h2>
      <span class="muted">${favorites.length} saved</span>
    </div>
    <div class="favorite-list">
      ${favorites.map(g => {
        ...
        return `<div class="favorite-card" onclick="selectRiver('${g.river || findGauge(g.id)?.river || ""}')">
          <strong>${g.name}</strong>
          <span>${fmtFlow(f?.value)}${Number.isFinite(t?.value) ? " • " + fmtTemp(t.value) : ""}</span>
        </div>`;
      }).join("")}
    </div>
  `;
...
function renderRiverCards(){
  ...
  const rivers = state.favoritesOnly
    ? RIVERS.filter(r => getRiverGauges(r).some(g => state.favorites.includes(g.id)))
    : RIVERS;

  grid.innerHTML = rivers.map(r => {
    const gauges = getRiverGauges(r);
    const headline = getHeadlineGauge(r);
    const d = state.data[headline?.id] || {};
    const f = latest(d.flow || []);
    const t = latest(d.temp || []);
    const st = tempStatus(t?.value);
    const favs = gauges.filter(g => state.favorites.includes(g.id)).length;
    const sectionText = r.sections ? r.sections.filter(s => !s.tributary).map(s => s.name.replace(r.name, "").trim()).join(" • ") : "No sections";
    ...
    return `<article class="river-card ${r.name===state.selected?'selected':''}" onclick="selectRiver('${r.name}')">
      ...
      <div class="river-sub">Headline: ${headline?.name?.replace(", MT","") || "—"}</div>
      <div class="river-metrics">
        <div class="mini"><div class="label">Flow</div><div class="value">${fmtFlow(f?.value)}</div></div>
        <div class="mini"><div class="label">Temp</div><div class="value ${st.cls}">${fmtTemp(t?.value)}</div></div>
      </div>
      <div class="river-sub" style="margin-top:12px;">${gauges.length} gauges${r.sections ? " • " + sectionText : ""}</div>
    </article>`;
  }).join("") || `<div class="empty panel">No favorite gauges yet. Star a gauge below.</div>`;
}
...
function renderDetails(){
  ...
  root.innerHTML = `
    <div class="section-title">
      <div>
        <h2>${river.name}</h2>
        <div class="section-subtitle">${getRiverGauges(river).length} curated gauges • live 7-day USGS flow and water temperature</div>
      </div>
      <a class="pill" target="_blank" rel="noopener" href="https://radar.weather.gov/">Radar ↗</a>
    </div>
    ${renderHatchPanel(river)}
    <div class="gauge-grid">
      ${river.sections ? renderSectionedGauges(river) : getRiverGauges(river).map(g => gaugeHtml(g, river.name)).join("")}
    </div>
  `;

  getRiverGauges(river).forEach(g => drawGauge(g.id));
}

function renderSectionedGauges(river){
  return river.sections.map(section => `
    <div class="group-title ${section.tributary ? "tributary" : ""}">${section.tributary ? "↳ " : ""}${section.name}</div>
    ${section.gauges.map(g => gaugeHtml({...g, tributary:!!section.tributary}, river.name)).join("")}
  `).join("");
}

function gaugeHtml(g, river){
  const d = state.data[g.id] || {};
  const f = latest(d.flow || []);
  const t = latest(d.temp || []);
  ...
  return `<article class="panel gauge" id="gauge-${g.id}">
    <div class="gauge-head">
      <div>
        <h3>${g.name}</h3>
        <div class="muted">${g.type ? g.type.charAt(0).toUpperCase() + g.type.slice(1) : "Gauge"} • Updated: ${fmtTime(f?.time)}</div>
      </div>
...
async function loadGauge(g){
  try{
    ...
    state.data[g.id] = {
      ...
      error: flow.status !== "fulfilled"
    };
  }catch(e){
    state.data[g.id] = {flow:[],temp:[],error:true};
  }
}
